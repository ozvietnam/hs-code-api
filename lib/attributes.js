...
// Gộp các field free-text của hồ sơ M0 thành 1 haystack đã fold để dò detect.
function haystack(attrs = {}) {
  return fold(
    [attrs.tenHang, attrs.chatLieu, attrs.congDung, attrs.chucNang, attrs.specs, attrs.nameZh, attrs.hinhDang]
      .filter(Boolean)
      .join(' • ')
  );
}
...
//   3. một regex detect khớp haystack
function isPresent(canonical, attrs = {}, hay) {
  const reg = registry();
  ...
  const h = hay != null ? hay : haystack(attrs);
