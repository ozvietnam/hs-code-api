...
/** Gộp nhóm Nice — ingest không được ghi đè (ca Kamoer: [7,9,11] bị thay bằng [35]). */
function mergeNiceClasses(...lists) {
  const out = new Set();
...
/** Nhóm hàng hoá (1–34) dùng để cross-check chương HS. */
function goodsClassesOf(entry = {}) {
  const fromGoods = mergeNiceClasses(entry.goodsNiceClasses);
  if (fromGoods.length) return fromGoods;
  return mergeNiceClasses(entry.niceClasses).filter((c) => c < 35);
}
...
function scoreMark(entry, hsCode) {
  ...
  const goodsClasses = goodsClassesOf(entry);
  let registeredChapters = hsChaptersForNice(goodsClasses);
  if (!registeredChapters.length && entry.hsChapters && entry.hsChapters.length) {
    // hsChapters đã suy từ loại hàng (vd bơm → ch.84) dù registry chỉ có class 35 dịch vụ
    registeredChapters = [...entry.hsChapters];
  }

  // classMatch: true = cùng nhóm hàng; false = khác nhóm; null = không xác định được
  let classMatch = null;
  if (chapter && registeredChapters.length) {
