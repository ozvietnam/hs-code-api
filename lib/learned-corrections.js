...
// Đọc qua data-paths (rule #8): đọc được bản feedback vừa ghi ở HS_DATA_DIR.
const { dataReadPath } = require('./data-paths');
const PROMOTIONS_PATH = dataReadPath('pattern-promotions.jsonl');
const FEEDBACK_PATH   = dataReadPath('feedback.jsonl');
 ...
 *   and expose learnedPenalty.correctedTo (mã từng được sửa thành) để người duyệt xem.
 * LƯU Ý: phạt theo MÃ, không theo mô tả hàng — một lần sửa A→B cho sản phẩm X
 * sẽ trừ điểm A cho mọi sản phẩm. Chưa chèn mã B vào danh sách gợi ý.
