...
async function callLLMJson(systemPrompt, userPrompt, opts = {}) {
  ...
  // Có BẤT KỲ provider fallback nào (kể cả Hermes) thì Gemini lỗi phải rớt xuống
  // fallback, không ném lỗi. Trước đây bỏ sót HERMES_API_KEY.
  const hasMinimax = FALLBACK_PROVIDERS.some((p) => Boolean(process.env[p.envKey]));
