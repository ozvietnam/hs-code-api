...
const TAX_PATH = dataPath('tax.json');
...
function buildEntryFromFile({ fileName, versionId, source, type }) {
  const full = snapshotReadPath(fileName);
...
function snapshotCurrentTax({ label = '', id = '', setCurrent = false, source } = {}) {
  ensureWritable();
  const liveTaxPath = dataReadPath('tax.json');
  if (!fs.existsSync(liveTaxPath)) {
  ...
  fs.copyFileSync(liveTaxPath, dest);
...
function rollbackToSnapshot(toFile, { backup = true } = {}) {
  ...
  const snap = snapshotReadPath(meta.file);
  if (!fs.existsSync(snap)) throw new Error(`Snapshot file missing: ${meta.file}`);
  const liveTaxPath = dataReadPath('tax.json');
  if (!fs.existsSync(liveTaxPath)) throw new Error('data/tax.json not found');

  let backupPath = null;
  if (backup) {
    backupPath = `${TAX_PATH}.bak-${Date.now()}`;
    fs.copyFileSync(liveTaxPath, backupPath);
  }
