...
function coerceEnumFact(inp, answer) {
  ...
  for (const [value, phrases] of Object.entries(inp._detect || {})) {
    if (phrases.some((p) => (p.dia ? raw : fold).includes(` ${p.key} `))) hits.push(value);
  }
...
module.exports = { coerceEnumFact, optionsViOf, loadTable, listTables, resolveHeading, extractFacts, toResolverShape, tableCoverage, validateTable, _reset };
