  ...
  const inputs = (raw.inputs || []).map((inp) => {
    ...
    for (const [value, phrases] of Object.entries(inp.detect || {})) {
      detect[value] = (phrases || []).map((p) => ({
        raw: p,
        dia: hasDiacritics(p),
        key: tidy(p, hasDiacritics(p)).trim(),
      }));
    }
    return {
      ...inp,
  ...
  for (const inp of table.inputs) {
    ...
    if (facts && facts[a] != null && String(facts[a]).trim() !== '') {
      if (inp.type === 'number') {
        ...
        const v = String(facts[a]).trim();
        if (!inp.domain || inp.domain.includes(v)) {
          out[a] = v;
          source[a] = 'explicit';
          continue;
        }
    ...
    if (inp.type === 'enum') {
      ...
      for (const [value, phrases] of Object.entries(inp._detect || {})) {
        if (phrases.some((p) => (p.dia ? raw : folded).includes(` ${p.key} `))) hits.push(value);
      }
      if (hits.length === 1) {
        out[a] = hits[0];
        source[a] = 'detect';
      ...
      // hai giá trị cùng xuất hiện → mâu thuẫn, coi như chưa biết để hỏi
    }
    // 3. thông số parser
...
function resolveHeading(heading, input = {}) {
  ...
  const { facts, source } = extractFacts(table, input);
...
module.exports = { loadTable, listTables, resolveHeading, extractFacts, toResolverShape, tableCoverage, validateTable, _reset };
