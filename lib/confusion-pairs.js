...
function confusionAlertsFor(text, candidateHs = [], { maxInfo = 3 } = {}) {
  ...
  const alerts = [];
  const seen = new Set();
  for (const { entry, matchedAlias } of matchByText(text)) {
    seen.add(entry.id);
    ...
    alerts.push(publicView(entry, { severity, matchedBy: 'text', matchedAlias, candidateHs: top, candidateInDeclared: inDeclared, candidateInCorrect: inCorrect }));
  }
  if (top) {
    let n = 0;
