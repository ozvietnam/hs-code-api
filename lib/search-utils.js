...
// Không lấy "dùng trong/dùng để/dùng làm": "Đồ dùng trong văn phòng" là tên hàng.
const USAGE_MARKER_RE = /\b(?:thích hợp )?(?:dùng cho|sử dụng cho|lắp vào|dành cho|chuyên dùng cho|của)\b/;
const USAGE_WEIGHT = 0.4;
function headOfLine(text) {
  const m = USAGE_MARKER_RE.exec(text);
  if (!m) return text;
  // Dòng bộ phận bắt đầu ngay bằng mốc ("- - Của máy thuộc nhóm 84.65", "Dùng cho
  // máy…") không có tên hàng riêng: cả dòng LÀ định nghĩa, không hạ điểm.
  const head = text.slice(0, m.index).replace(/[-\s]+/g, ' ').trim();
  return head.length < 3 ? text : text.slice(0, m.index);
}
