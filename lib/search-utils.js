...
function searchCandidates(query, { topCandidates = 10, csOnly = false } = {}) {
  ...
  scoreMeta.contextHits = isHSQuery ? null : contextHitCounts(keywordTokens);
  ...
  const countedTokens = keywordTokens.filter((w) => w.length >= 3).length || 1;
    ...
    .map((item) => {
      ...
      if (!isHSQuery) {
        ...
        if (!hit) {
          ...
          if (ctxHits >= 2 && ctxHits / countedTokens >= 0.4) hit = true;
        }
        if (!hit) return null;
      }
