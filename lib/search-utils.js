...
function searchCandidates(query, { topCandidates = 10, csOnly = false, facts = null } = {}) {
  ...
  // của THUỘC TÍNH (dày, Ø, tự hành…), không phải của tên. Xét tối đa 3 nhóm
  // đang dẫn đầu; nhóm nào có bảng thì phân giải bằng dữ kiện bóc từ câu
  // (+ facts tường minh nếu người gọi đưa). Chốt được lá → lá đó lên đầu;
  // thiếu dữ kiện → trả câu hỏi đúng dữ kiện đó thay vì đoán.
  const decisions = [];
  {
    const byScore = [...merged].sort((a, b) => b.score - a.score);
    const headings = [];
    for (const c of byScore) {
      const h = c.hsCode.slice(0, 4);
