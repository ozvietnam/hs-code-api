...
function determineGir(input = {}) {
  ...
  // 1) Bảng quyết định do người soạn — căn cứ mạnh nhất CHỈ khi bảng đã verified.
  // Bảng cộng đồng chưa kiểm chứng không được đội lốt RULE_TABLE (D-2).
  if (resolver?.status === 'RESOLVED' && resolver.gir) {
    const tableVerified = resolver.tableVerified === true;
    out.push(
      determination(normalizeRuleKey(resolver.gir) || '1', {
        basis: tableVerified ? BASIS.RULE_TABLE : BASIS.HEURISTIC,
