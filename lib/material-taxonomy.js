...
const TAXONOMY_FILES = ['polymers', 'woods', 'fibers', 'chemicals', 'metals'];
...
function buildIndex() {
  ...
  for (const file of TAXONOMY_FILES) {
    ...
    for (const [key, meta] of Object.entries(data)) {
      ...
      if (meta.nameVi) {
        mergedIndex.push({
          key: String(meta.nameVi).toLowerCase(),
          family: file,
          aliasOf: key,
          ...meta,
        });
...
// A3 (#45) — kiểm mọi hsHints là PREFIX hợp lệ (khớp ≥1 mã 8-số trong tax.json).
// Hint có thể 2/4/6 số (chương/nhóm/phân nhóm). Deterministic, dùng để khoá data + test.
function validateHsHints() {
  const taxKeys = Object.keys(require('../data/tax.json')).filter((k) => k.length === 8);
  const invalid = [];
  let total = 0;
  for (const file of TAXONOMY_FILES) {
    const data = loadTaxonomyFile(file);
    for (const [material, meta] of Object.entries(data)) {
      for (const hint of meta.hsHints || []) {
        total += 1;
        const prefix = String(hint).replace(/\D/g, '');
        if (!prefix || !taxKeys.some((k) => k.startsWith(prefix))) {
          invalid.push({ family: file, material, hint });
        }
      }
    }
  }
  return { total, invalidCount: invalid.length, invalid };
}
