...
function walk(node, path, acc) {
  if (typeof node === 'string') {
    // source.url: id bài viết trong đường dẫn không phải MST/số tờ khai — che
    // các dãy số dài trước khi quét, mẫu chữ (tên công ty…) vẫn quét như thường.
    const value = /(^|\.)source\.url$/.test(path) && /^https?:\/\//.test(node)
      ? node.replace(/\d{6,}/g, (d) => 'x'.repeat(d.length))
      : node;
    for (const hit of scanString(value)) acc.push({ path: path || '(gốc)', ...hit });
    return;
  }
