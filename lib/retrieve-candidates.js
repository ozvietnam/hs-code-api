...
const { filterChapter98 } = require('./chapter98');
...
async function getCandidates(attrs, opts = {}) {
  ...
  const allHeadings = [...score.values()]
  ...
  // Chương 98 = mã ưu đãi riêng, không phải kết luận phân loại. Đo trên benchmark
  // 200 tờ khai thật: 10% lỗi là do đoán vào chương 98, trong khi 0 tờ khai có
  // đáp án thật thuộc chương này. Xem lib/chapter98.js.
  const h = filterChapter98(allHeadings, {
    getHs: (x) => x.code4,
    query: attrs?.tenHang || attrs?.description || '',
    include: opts.includeChapter98,
  });
  const codes = filterChapter98(prec.codes, {
    query: attrs?.tenHang || attrs?.description || '',
    include: opts.includeChapter98,
  });
  ...
  return {
    headings: h.items,
    precedentCodes: codes.items,
    chapter98Filtered: h.removed + codes.removed,
  };
}
