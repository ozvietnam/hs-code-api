...
async function llmProposeHeadings(attrs, opts = {}) {
  ...
  const { content } = await chat(
    [{ role: 'system', content: SYS_CANDIDATES }, { role: 'user', content: `Sản phẩm: ${q}` }],
    { json: true, timeoutMs: opts.timeoutMs || 40000 },
  );
  let s = String(content).replace(/<think>[\s\S]*?<\/think>/gi, '').replace(/```json?/gi, '').trim();
  const m = s.match(/\{[\s\S]*\}/);
