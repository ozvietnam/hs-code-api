...
// B2 (#48) — Tra taxonomy từ chatLieu → hsHints (nhóm 4-số) làm ứng viên bổ sung.
// Deterministic, không cần LLM, chạy song song với llmProposeHeadings.
function taxonomyHeadingsFromAttrs(attrs) {
  if (!attrs.chatLieu) return [];
  const mats = detectMaterials(attrs.chatLieu);
  const seen = new Set();
  const result = [];
  for (const m of mats) {
    for (const hint of m.hsHints || []) {
      const h4 = String(hint).replace(/\D/g, '').slice(0, 4);
      if (h4.length === 4 && !seen.has(h4)) { seen.add(h4); result.push(h4); }
    }
  }
  return result;
}
 ...
 * cộng tín hiệu nếu precedent cũng trỏ tới (Oz từng khai nhóm đó) hoặc taxonomy khớp chatLieu.
 */
async function getCandidates(attrs, opts = {}) {
  ...
  const taxoHeadings = taxonomyHeadingsFromAttrs(attrs);
