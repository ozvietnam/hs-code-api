 ...
 *   · gắn nhãn "[chưa kiểm chứng]" cho số hiệu văn bản LLM tự trích (citation-guard);
 *   · trả danh sách mã bị loại để minh bạch.
 */

const { annotateField } = require('./citation-guard');
 ...
 * @param {string}   ctx.contextText dữ liệu đã đưa LLM — để kiểm số hiệu văn bản LLM trích
 * @returns {{suggestions:object[], rejected:{hsCode:string, reason:string}[]}}
 */
function sanitizeLlmSuggestions(raw, { evidence, taxData, limit = 3, contextText = '' }) {
  ...
  for (const s of Array.isArray(raw) ? raw : []) {
    ...
    // Số hiệu TT/NĐ LLM tự viết mà không có trong chỉ mục/dữ liệu → gắn nhãn.
    suggestions.push(annotateField(clean, 'reasoning', contextText));
