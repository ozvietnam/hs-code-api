...
const { llmStatus } = require('./llm-tier');
...
function buildAdminOverview() {
  ...
  return {
    ...
    systemHealth: {
      // Cùng tiêu chí với /api/health: thiếu LLM là xuống cấp thật, không tô hồng.
      status: total > 0 && apiToken && llm.ok ? 'healthy' : 'degraded',
      service: 'hs-code-api',
      version: '2.0.0',
      checks: {
        taxData: { ok: total > 0, rows: total },
        llm,
