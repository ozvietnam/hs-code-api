...
const { getByHs } = require('./explanatory-notes.js');
const { conflictsData, chapterRulesData } = require('./data');
...
function buildContext(attrs, headings) {
  ...
  for (const h4 of headings) {
    ...
    // B1 (#47) — điểm phân biệt + tính chất: chỉ thêm nếu có (field rỗng khi A1 chưa enrich)
    if (n.phanBiet) lines.push(`  PHÂN BIỆT: ${clip(n.phanBiet, 200)}`);
    if (n.tinhChat) lines.push(`  TÍNH CHẤT: ${clip(n.tinhChat, 160)}`);
...
async function classify(attrs, opts = {}) {
  ...
  // B2 (#48) — chapter-specific checklist: trường bắt buộc theo chương chưa có trong hồ sơ
  const topChapter = (top?.hs || headings[0]?.code4 || '').slice(0, 2);
  const chapRule = chapterRulesData.chapters?.[topChapter];
  if (chapRule?.chapterSpecificRequired?.length) {
    for (const field of chapRule.chapterSpecificRequired) {
      if (!attrs[field]) missing.push(`[ch.${topChapter}] Bổ sung: ${field}`);
    }
  }

  // B3 (#49) — explanatory-note + conflict warning cho mã top
  const warnings = [];
  if (top && top.hs.length === 8) {
    const note = getByHs(top.hs);
    if (note?.noteVi) top.explanatoryNote = note.noteVi;

    const conflict = conflictsData[top.hs];
    if (conflict?.confusedWith?.length) {
      const reasons = conflict.reasonsVi?.length ? ` — ${conflict.reasonsVi[0]}` : '';
      warnings.push(`Mã ${top.hs} dễ nhầm với: ${conflict.confusedWith.join(', ')}${reasons}`);
    }
  }
