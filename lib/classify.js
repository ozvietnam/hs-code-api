...
⚡ SUY LUẬN NGẮN GỌN: đủ căn cứ chú giải là KẾT LUẬN + xuất JSON NGAY, KHÔNG nghĩ lan man dài dòng.
...
function buildContext(attrs, headings) {
  ...
  for (const h4 of headings) {
    ...
    if (n.chuong) chapLines.push(`■ Chương ${ch2}: ${clip(n.chuong, 200)}`);
  }
  if (chapLines.length) {
...
async function girConfirm(attrs, headings, opts = {}) {
  ...
  const { json, provider } = await callLLMJson(SYS_GIR, ctx, { tier: opts.tier, maxTokens: 2800, timeoutMs: opts.timeoutMs || 40000 });
