...
async function girConfirm(attrs, headings, opts = {}) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      const { json, provider } = await callLLMJson(SYS_GIR, ctx, { tier: opts.tier, maxTokens: 4000, timeoutMs: opts.timeoutMs || 60000 });
...
async function classify(attrs, opts = {}) {
  ...
  const { results, missing, provider } = await girConfirm(attrs, headings.map((h) => h.code4), opts);
