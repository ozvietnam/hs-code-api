...
async function classify(attrs, opts = {}) {
  ...
  // B2: chapter-specific-rules — bổ sung missing[] theo checklist chương
  if (top?.hs) {
    const ch2 = nz(top.hs).slice(0, 2);
    const rule = chapterRules()[ch2];
    if (rule) {
      const knownFields = new Set(['tenHang', 'chatLieu', 'congDung', 'chucNang', 'specs', 'nameZh', ...Object.keys(attrs)]);
      const extraMissing = (rule.chapterSpecificRequired || [])
        .filter((f) => !knownFields.has(f) || !attrs[f])
        .map((f) => `[Ch.${ch2}] ${rule.girHints?.[0] || `Cần ${f}`}`);
  ...
  if (top?.hs?.length === 8) {
    explanatoryNote = getNoteSummaryForHs(top.hs);
    const conflict = conflictsDb()[top.hs];
