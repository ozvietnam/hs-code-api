...
async function classify(attrs, opts = {}) {
  ...
  // B2: chapter-specific-rules
  if (top?.hs && lowConf) {
    const ch2 = nz(top.hs).slice(0, 2);
    const rule = chapterRules()[ch2];
    if (rule) {
      const registry = attributesDb();
      const knownFields = new Set(Object.keys(attrs));
      for (const [canon, def] of Object.entries(registry)) {
        knownFields.add(canon);
        if (def.aliasVi) knownFields.add(def.aliasVi);
      }
      const chapterFieldAliases = { function: 'chucNang', material: 'chatLieu', purpose: 'congDung' };
      const extraMissing = (rule.chapterSpecificRequired || [])
        .filter((f) => {
          const resolved = chapterFieldAliases[f] || f;
          return !knownFields.has(f) && !attrs[resolved];
        })
        .map((f) => `[Ch.${ch2}] Cần thêm: ${f}`);
  ...
  return {
    ...
    resolver,          // kết quả bảng quyết định deterministic (B3.5)
...
module.exports = { classify, girConfirm, getCandidates, headingNotes, tbTchqGate, conflictsDb, attributesDb, conflictTablesDb };
