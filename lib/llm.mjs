...
function buildProviders() {
  return {
    ...
    hermes: {
      ...
      model: env('HERMES_ENRICH_MODEL') || 'reasoning',
      style: 'openai',
    },
    minimax: {
...
// Ưu tiên Hermes Pool (cổng dùng chung) → MiniMax → OpenRouter free. Gemini KHÔNG có ở đây — guard.
// Provider nào thiếu key sẽ tự bị skip (vd prod Vercel chưa set HERMES_API_KEY).
const DEFAULT_ORDER = ['hermes', 'minimax', 'openrouter'];
