...
function buildProviders() {
  return {
    byteplus: {
      // BytePlus ModelArk (ByteDance) — OpenAI-compatible. Region quốc tế mặc định;
      // đổi BYTEPLUS_BASE_URL nếu account ở region khác. BYTEPLUS_ENRICH_MODEL = model/endpoint-id
      // đã bật trên account (vd ep-xxx hoặc tên model foundation).
      baseUrl: env('BYTEPLUS_BASE_URL') || 'https://ark.ap-southeast.bytepluses.com/api/v3',
      key: env('BYTEPLUS_API_KEY'),
      model: env('BYTEPLUS_ENRICH_MODEL') || env('BYTEPLUS_ENDPOINT_ID') || 'DeepSeek-V4-Flash',
      style: 'openai',
    },
    minimax: {
...
// Ollama đã gỡ (làm chậm máy CEO 2026-05-29). BytePlus ưu tiên (CEO cấp 2026-06-02),
// fallback MiniMax → OpenRouter free. Gemini KHÔNG có ở đây — guard.
const DEFAULT_ORDER = ['byteplus', 'minimax', 'openrouter'];
