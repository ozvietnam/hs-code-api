...
function applyGirRules(candidates, description) {
  ...
  if (filtered.some((c) => matchEssential(description, c.essentialCharacteristics) > 0)) {
    rankingSignals.push({
      ...
      note: 'Khớp từ khoá đặc tính — tín hiệu xếp hạng, KHÔNG phải căn cứ quy tắc phân loại.',
    });
  }
  ...
  if (
    ...
    rankingSignals.push({
      ...
      note: 'Hoà điểm → ưu tiên mã số học sau cùng. Có trích quy tắc hay không xem girRulesApplied.',
    });
  }
  ...
  if (ch && MIXTURE_CHAPTERS.has(ch) && /\d+\s*%|hoa hop|hon hop|mixture/i.test(description)) {
    rankingSignals.push({
      ...
      note: 'Có dấu hiệu hàng hỗn hợp — cần xét quy tắc về hỗn hợp/bộ; chưa đủ để kết luận.',
    });
  }
