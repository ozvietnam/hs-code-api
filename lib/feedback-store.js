...
const PROMOTIONS_PATH = dataPath('pattern-promotions.jsonl');
...
function listFeedback({
  ...
  if (dateFrom) {
    ...
    if (!Number.isNaN(from.valueOf())) {
      rows = rows.filter((r) => new Date(r.createdAt || r.receivedAt || 0) >= from);
    }
  }
  if (dateTo) {
    const to = new Date(dateTo);
    if (!Number.isNaN(to.valueOf())) {
      rows = rows.filter((r) => new Date(r.createdAt || r.receivedAt || 0) <= to);
    }
  }
  const sortKey = sortBy === 'status' ? 'status' : sortBy === 'feedbackType' ? 'feedbackType' : 'createdAt';
  const dir = String(sortDir).toLowerCase() === 'asc' ? 1 : -1;
  rows.sort((a, b) => {
    const av = String(a[sortKey] || a.receivedAt || a.createdAt || '');
    const bv = String(b[sortKey] || b.receivedAt || b.createdAt || '');
...
function bulkApproveByPattern({ fromHs, toHs, reviewedBy }) {
  const records = readAll();
  const matched = records
    .filter(
      (r) =>
        (r.status || 'pending') === 'pending' &&
        String(r.hsCodeAtTime || '') === String(fromHs || '') &&
        String(r.correctedHsCode || '') === String(toHs || ''),
    )
    .map((r) => r.feedbackId);
  return bulkReviewFeedback(matched, { action: 'approve', reviewedBy });
}
