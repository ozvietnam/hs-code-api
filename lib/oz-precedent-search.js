...
const { isHeldOut, evalExcludesHoldout } = require('./holdout');
...
async function ensureCache() {
  ...
  // Đang chấm điểm (HS_EVAL_EXCLUDE_HOLDOUT=1): bỏ tập giữ riêng khỏi kho tiền
  // lệ — nếu không, mẫu đem chấm tự tìm thấy đáp án của chính nó.
  if (evalExcludesHoldout()) cachedGold = cachedGold.filter((g) => !isHeldOut(g));
...
// Sync stats cho /api/health — chứng minh gold data có trong lambda bundle (không cần auth).
function ozGoldStats() {
  if (GOLD_BLOB_URL) return { ok: true, source: 'blob', records: null };
  try {
    if (!fs.existsSync(GOLD_PATH)) return { ok: false, source: 'file', records: 0 };
    const text = fs.readFileSync(GOLD_PATH, 'utf8').trim();
    const records = text ? text.split('\n').length : 0;
    return { ok: records > 0, source: 'file', records };
  } catch (e) {
    return { ok: false, error: e.message };
  }
}

module.exports = { searchOzByHs, searchOzByKeyword, ozGoldStats };
