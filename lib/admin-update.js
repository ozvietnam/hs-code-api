...
const ENRICHED_PATH = dataPath('tax-enriched.json');
const AUDIT_PATH = dataPath('audit-log.jsonl');
...
function ensureWritable() {
  ...
  const probe = dataPath('.admin-write-probe');
  fs.writeFileSync(probe, 'ok', 'utf8');
  fs.unlinkSync(probe);
}

function loadEnrichedMap() {
  const readPath = dataReadPath('tax-enriched.json');
...
function applyAdminPatch(hsCode, patch, { comment = '', admin = 'admin' } = {}) {
  ...
  if (patch.nameVi !== undefined) {
    const taxMap = JSON.parse(fs.readFileSync(dataReadPath('tax.json'), 'utf8'));
    if (!taxMap[hs]) {
