...
function mapTaxRecord(record) {
  ...
  return {
    ...
    warnings: hasPolicy
      ? {
          requiresLicense: /giấy phép|giay phep/i.test(record.cs),
          requiresInspection: /kiểm tra|kiem tra|CR\b/i.test(record.cs),
          requiresQuarantine: /kiểm dịch|kiem dich/i.test(record.cs),
          dualUseControl: /mật mã|mat ma|chuyên dụng|chuyen dung/i.test(record.cs),
          ministries: extractMinistries(record.cs),
          summary: summarizePolicy(record.cs),
        }
      : null,
  };
}
