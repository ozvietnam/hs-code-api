...
const precCodes = new Set((Array.isArray(precRaw) ? precRaw : Object.values(precRaw).flat()).map((x) => x.outcome).filter(Boolean)).size;
...
check(`AGENTS.md: tiền lệ (${fmt(precCodes)} mã)`, agents.includes(`Tiền lệ TB-TCHQ (${fmt(precCodes)} mã)`));
